...
export default ({ xAxisData, series = [], granularity, total }) => ({
  ...
  series: series.map((s) => ({ type: 'line', smooth: true, symbol: 'roundRect', symbolSize: 3, ...s })),
  tooltip: {
    enterable: false,
    confine: true,
    trigger: 'axis',
    ...
    formatter(params) {
      ...
      const totalTokens = params.reduce((sum, p) => sum + p.value, 0);
