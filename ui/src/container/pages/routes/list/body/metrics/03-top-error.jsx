...
import { useGetTopErrorRouteWithRange } from '@State/routes/vertical-hooks';
...
function TopError() {
  const { data, isLoading, isError } = useGetTopErrorRouteWithRange();
...
function IsEmpty() {
  return (
    <Board
      className="flex-1"
      footer={(
        <span className="flex text-sm text-gray-400" style={sparklineWidthAndHeight}>No data available</span>
      )}
      main={(
        <div className="flex justify-between gap-2 w-full">
          <NewHeader
            padding="none"
            title={(
              <SectionTitle subtitle="No data available" title="Top Error" />
            )}
          />
      ...
      main={(
        <div className="flex justify-between gap-2 w-full">
          <NewHeader
            padding="none"
            title={(
              <SectionTitle subtitle={<HtmlAnchor onClick={handleOnClickRouteName}>{name}</HtmlAnchor>} title="Top Error" />
            )}
          />
