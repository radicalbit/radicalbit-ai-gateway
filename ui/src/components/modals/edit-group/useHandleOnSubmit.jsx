...
import { useEditGroupMutation } from '@State/groups/api';
...
export default () => {
  ...
  const [trigger, args] = useEditGroupMutation({ fixedCacheKey: `edit-group-${uuid}` });
  ...
  const handleOnSubmit = useCallback(() => {
    ...
    submitForm(async ({ form: formData }, setError) => {
      const { error: editModelError } = await trigger({
        data: formData,
        uuid,
        successMessage: (<SuccessMessage prefix="Group" strong={formData.name} suffix="updated" />),
      });
