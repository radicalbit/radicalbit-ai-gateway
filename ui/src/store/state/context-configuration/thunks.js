...
const changeContextConfiguration = createAsyncThunk(
  ...
  async ({ namespace, configuration }, { getState, fulfillWithValue, dispatch, rejectWithValue }) => {
    try {
      const { pagination, sorter, filters, externalFilters } = configuration;
...
const changeExternalFilters = createAsyncThunk(
  ...
  async ({ namespace, externalFilters }, { getState, dispatch, fulfillWithValue }) => {
    const old = selectContextConfiguration(getState(), namespace);
