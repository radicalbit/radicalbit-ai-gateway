...
ORDER BY
  (ServiceName, TimestampTime, Timestamp) SETTINGS index_granularity = 8192,
...
PARTITION BY
  toDate (Timestamp)
ORDER BY
  (ServiceName, SpanName, toDateTime (Timestamp)) SETTINGS index_granularity = 8192,
  ttl_only_drop_parts = 1;
...
PARTITION BY
  toDate (Start)
ORDER BY
  (TraceId, Start) SETTINGS index_granularity = 8192,
  ttl_only_drop_parts = 1;

CREATE MATERIALIZED VIEW IF NOT EXISTS default.otel_traces_trace_id_ts_mv TO default.otel_traces_trace_id_ts (
  `TraceId` String,
  `Start` DateTime64 (9),
  `End` DateTime64 (9)
) AS
...
-- +goose Down
DROP VIEW IF EXISTS default.otel_traces_trace_id_ts_mv;
DROP TABLE IF EXISTS default.otel_traces_trace_id_ts;
